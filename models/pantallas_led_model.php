...
class Pantallas_led_Model extends Model {
    ...
    public function cargarPantallasLed($pagina) {
        $page = 1;
        if (!empty($pagina)) {
            $page = $pagina;
        }
        ...
        if (!empty($sql)) {
            foreach ($sql as $item) {
                $div .= '<!-- start portfolio-item item -->
                        <li class="grid-item web branding design wow fadeInUp" ' . $style . '>
                            <a href="#">
                                <figure>
                                    <div class="portfolio-img bg-extra-dark-gray"><img src="' . URL . 'public/images/pantallas_led/' . $item['imagen'] . '" alt=""/></div>
                                    <figcaption>
            ...
            $paginador = $this->helper->mostrarPaginador($setLimit, $page, 'pantallas_led_img', 'pantallas_led', 'cargarPantallasLed', $condicion, TRUE);
        }
        $data = array(
            'contenido' => $div,
            'paginador' => $paginador
